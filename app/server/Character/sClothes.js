...
async function saveClothes(player, d) {
	...
	switch (d.title) {
		case "Hats":
			await misc.query(`UPDATE usersClothes SET hats = '${JSON.stringify(obj)}' WHERE id = ${player.basic.id}`);
			break;

		case "Glasses":
			await misc.query(`UPDATE usersClothes SET glasses = '${JSON.stringify(obj)}' WHERE id = ${player.basic.id}`);
			break;

		case "Tops":
			obj.underColor = d.underColor;
			await misc.query(`UPDATE usersClothes SET tops = '${JSON.stringify(obj)}' WHERE id = ${player.basic.id}`);
			break;

		case "Legs":
			await misc.query(`UPDATE usersClothes SET legs = '${JSON.stringify(obj)}' WHERE id = ${player.basic.id}`);
			break;

		case "Feet":
			await misc.query(`UPDATE usersClothes SET feet = '${JSON.stringify(obj)}' WHERE id = ${player.basic.id}`);
			break;
...
async function loadPlayerClothes(player) {
	const d = await misc.query(`SELECT hats, glasses, tops, legs, feet FROM usersClothes WHERE id = '${player.basic.id}'`);
...
async function insertNewUser() {
	...
	await misc.query(`INSERT INTO usersClothes (hats, glasses, tops, legs, feet) VALUES ('${obj}', '${obj}', '${tops}', '${obj}', '${obj}')`);
}
